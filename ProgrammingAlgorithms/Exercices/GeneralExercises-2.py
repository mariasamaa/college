...
def inicio_fim_igual(nums):
    # Retorna True se a lista nums possui pelo menos
    # um elemento e o primeiro elemento é igual ao último
...
def extremidades_iguais(a, b):
    ...
    # iguais ou os dois últimos são iguais. Suponha que as listas
...
def maior_ponta(nums):
    ...
    # os elementos são o maior das duas pontas obs.: não é o maior
    # de todos os itens, mas entre as duas pontas
...
def soma_2_primeiros(nums):
    ...
    # retorna a soma dos dois primeiros elementos. Se
    # a lista tiver menos de dois elementos, soma o que
    # for possível.
...
def meio_do_caminho(a, b):
    # Sejam duas listas de inteiros a e b retorna uma
    # lista de tamanho 2 contendo os elementos do meio
    # de a e b, suponha que as listas tem tamanho ímpar
...
def gago(texto):
    # Receba um texto e devolva-o repetindo a primeira letra de cada
    # palavra, junto com um traço
    ...
    for i, palavra in enumerate(palavras):
        palavras[i] = f"{palavra[0]}-{palavra}"
...
def rosquinhas(n):
    # Para um inteiro n retorna uma string na forma '<n>
    # rosquinhas, onde n é o valor passado como argumento.
    # Caso n >= 10 devo retornar 'muitas' em lugar do número.
    # rosquinhas(5) -> '5 rosquinhas'
    # rosquinhas(23) -> 'muitas rosquinhas'

    if n < 10:
        return (f'{n} rosquinhas')
    return (f'muitas rosquinhas')


def pontas(s):
    # Dada uma string s, retorna uma string com as duas
    # primeiras e as duas últimas letras da string
    # original s (Assim 'palmeiras' retorna 'paas').
    # No entanto, se a string tiver menos que 2 letras,
...
def nomes_pontas(n):
    # Dada uma string n contendo o nome completo de
    # uma pessoa, retorne uma nova string contendo o
    # primeiro e o último nome, em maiúsculas.
...
def mistura2(a, b):
    # Sejam duas strings a e b. Retorno uma string
    # '<a> <b>' separada por um espaço com as duas
    # letras trocadas de cada string
    # 'mix', pod' -> 'pox mid'
...
def tres_maiusculas(texto):
    # Encontre a primeira ocorrência de 3 letras maiúsculas
    ...
    if len(posicoes) < 3:
        return None
    elif not (posicoes[0] + 1 == posicoes[1] and posicoes[1] + 1 == posicoes[2]):
        return None
    else:
        return posicoes[0]

def im_pares_unicos(lista):
    # Separe em listas os impares e pares únicos (não
    # considere duplicidades), dos inteiros da 'lista'
    # index == lista.index(numero) se a posicao do numero for
    # diferente da primeira ocorrencia, é uma duplicata;

    pares = [n for n in lista if n % 2 == 0]
    impares = [n for n in lista if n % 2 != 0]

    # set = remove as duplicadas de uma lista
    return list(set(pares)), list(set(impares))
...
def main():
    ...
    test(gago("eu deveria ter estudado mais"),
         "e-eu d-deveria t-ter e-estudado m-mais")
    ...
    test(
        nomes_pontas_e_iniciais_do_meio(
            "Marco André Lopes Mendes"), "MARCO A L MENDES"
    )
    ...
    test(nomes_pontas_e_iniciais_do_meio(
        "Paulo César de Oliveira"), "PAULO C OLIVEIRA")
    test(nomes_pontas_e_iniciais_do_meio(
        "Fernando José Braz"), "FERNANDO J BRAZ")
