...
def perto_de_10(n):
    # Seja um n não negativo, retorna True se o número está a
    # distância de pelo menos dois de um múltiplo de dez.
    # Dica: use a função resto da divisão.
    # perto_de_10(12) -> True
    # perto_de_10(17) -> False
    # perto_de_10(19) -> True

    return abs((10 * (n // 10)) + - n) <= 2 or abs((10 * (n // 10 + 1) - n)) <= 2
...
def conta_bola(s):
    ...
    contador = 0
    for i, _ in enumerate(s):
        if s[i] == "a" and s[i-2] == "o" and s[i-3] == "b":
            contador += 1
    return contador
