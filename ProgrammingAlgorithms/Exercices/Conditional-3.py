# Lista de exercício de repetição FOR
# # ATENÇÃO !!!
...
def soma_das_temperaturas(lista):
    ...
    #Argumentos: lista (lista de floats) = Uma lista de valores float.
    #Retorna: float = a soma_das_temperaturas dos elementos da lista.
...
def quantidade_de_impares(valor_inicial, valor_final):
    ...
    #Retorna: int = a quantidade de números ímpares dentro do intervalo dado.
...
def soma_das_temperaturas_dos_inteiros(valor1, valor2):
    ...
    #Considere que os limites do intervalo podem ser informados como números
    #negativos ou fora de ordem. Ex: 1 e 5 ou 5 e 1, retorna 9

    #Argumentos:
        #valor1 (int): um dos limite do intervalo, excluindo-o;
        #valor2 (int): o outro limite do intervalo, excluindo-o;
    #Retorna: float = a soma dos valores dentro do intervalo dado.
...
def serie(n):
    ...
    #s = 1 + 1/2 + 1/3 + 1/4 + ... + 1/n
    #Argumento: n (int) = o valor final da série;
    #Retorna: float= a soma dos valores da série,
     #segundo a fórmula e o valor de n informados.

    s = 0
    for i in range(1,n):
         s += 1/i
    return (round(s+1/n,2))

def ordenamento_contrario(lista):
    #Inverta a ordem dos elemntos da lista.
    #Argumento: lista (list) =uma lista de elementos, independente de tipo.
    #Retorna: list = uma lista com os elementos a ordem invertida.
...
def intercalamento_listas(lista1, lista2):
    ...
    #crie uma nova lista composta pelo intercalamento entre 
    #as duas. Intercalar significa que a nova lista terá o 1o
    #elemento da 1a lista, seguido do 1o elemento da 2a lista,
    #seguido do 2o elemento da 1o lista, seguido do 2o elemento
    #da 2a lista e assim por diante.

    #Argumentos:
        #lista1 (list): uma lista de elementos, independente de tipo;
        #lista2 (list): uma lista de elementos, independente de tipo;

    #Retorna: list = uma lista com os elementos intercalados das duas 
    #listas recebidas.
...
def im_pares(lista):
    # Separe em listas os pares e impares, dos inteiros da lista 
    # recebida.
    # Argumento: lista (lista de inteiros) = uma lista de elementos int;
    # Retorna:uma tupla com duas listas de inteiros sendo a primeira uma 
    #lista com os pares e a segunda uma lista com os ímpares.
...
def maior_menor(lista):
    ...
    #Retorna: uma tupla com dois números inteiros, o maior e
    #o menor da lista.
...
def dar_troco(valor_a_pagar, valor_em_dinheiro):
    ...
    #A resposta deve conter a quantidade de cada nota, sem considerar
    #centavos.
    ...
    #Retorna:
        ...
        #Se a quantidade de notas for igual a zero, não deve
        #aparecer na lista.

        cedula = (50,20,10,5,2,1)
        ...
        if troco == 0:
            ...
            for i in cedula:
                quantidade = troco // i
                troco = troco - (i*quantidade)
                if quantidade > 0:
                    #append aceita só uma unidade, pra por duas tem que deixar
                    #junto entre ()
...
def media_anual(temperaturas):
    ...
    #Argumentos: temperaturas (lista de floats): as temperaturas
    #médias de cada mês no ano, em ordem.

    #Retorna: lista de inteiros: uma lista com o número correspondente ao mês em que a
    #temperatura média foi maior que a temperatura média anual.

    media = round((sum(temperaturas)/len(temperaturas)),2)
    meses = []
    for n,i in enumerate(temperaturas):
        if i > media:
            meses.append(n)
    return meses        
...
def maiores_13(idades, alturas):
    ...
    #listas separadas e de igual comprimento. Calcule a media
    #das alturas e retorne as alturas daqueles que possuem 
    ...
    #Retorna: uma lista de alturas dos alunos, conforme o criério
    #definido.

    media = (sum(alturas)) / (len(alturas))
    aluno = []

    for n,i in enumerate (idades):
        if alturas[n] < media and i > 13:
            aluno.append(alturas[n])
    return aluno
            
    
def testa_primo(valor):
    #Recebe um valor e verifica se ele é um número primo ou não.
    #Argumento: valor (int): um número inteiro.
    #Retorna: bool: True ou False, se o valor e ou não primo.
...
def lista_de_primos(inicio, fim):
    ...
    #Retorna:
        #lista de inteiros, os primos dentro do intervalo
        #especificado.
    ...
    for i in range (inicio,fim+1):
        if testa_primo(i):
            primos.append(i)
    return primos
    #metodo é como é chamado uma ferramenta como append
    #append é um método de lista, adicionar um item
...
def fibonacci(n):
    ...
    for i in range(n):
        if len(lista) <= 1:
            ...
            #último i + penultimo i = proximo i
...
def altera_salarios(salarios):
    ...
    #Argumento: salarios (lista de floats): os salários originais.
    #Retorna: uma lista de elementos float, correspondendo aos
    #salários corrigidos.
    ...
    for i in salarios:
        if i <= salario_min:
            salarios_novo.append(round((i*1.20),2))
        elif i <= (salario_min * 2):
            salarios_novo.append(round((i*1.15),2))
        elif i <= (salario_min * 5):
            salarios_novo.append(round((i*1.10),2))
        else:
            salarios_novo.append(round((i*1.05),2))
    return salarios_novo
...
def main():
    ...
    test(
        altera_salarios([500, 724.0, 725.00, 1448.00, 1449.00, 3620.00, 3621.00, 4000.00]),
