...
def produto_cartesiano(homens, mulheres):
    ...
    # cartesiano, e para fazer isso em uma listcomp ou genexp você precisa
    # usar mais de um for dentro da expressão.
    return [(h, m) for h in homens for m in mulheres]
    

def produto_cartesiano_filtro(homens, mulheres):
    # Repita o exercício 5, acrescentando um filtro com if para remover
    # os nomes com menos de 4 letras das duas listas. Quantos casais
    # serão formados?
...
def gago(texto):
    # Receba um texto e devolva-o repetindo a primeira letra de
    # cada palavra, junto com um traço
...
def intercalamento_listas(lista1, lista2):
    ...
    # as duas.
    return [num for i in list(zip(lista1, lista2)) for num in i]
...
def numeros_sortudos(limite_inferior=1, limite_superior=100000):
    ...
    return len([i for i in range(limite_inferior, limite_superior+1) if "2" in str(i) and "7" not in str(i)])
...
def main():
    ...
    test(
        iniciais_e_nomes_incompleto(
            homens), {"P": "Pedro", "J": "Joaquim", "T": "Tom"}
    )
