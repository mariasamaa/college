...
def limpa_texto(texto):
    ...
    # Argumento: texto (string): um texto contendo letras, números,
    # sinais de pontuação e espaços em branco.
    # Retorna: string: uma string contendo apenas as letras.
...
def camel_casing(texto):
    # Camel Casing é uma forma de escrever nomes de identificadores,
    # (como varíaveis e funções) usando letras maiúsculas para separar
    # as palavras. A palavra "camel" em inglês significa camelo (lembre-se
    # das córcovas do camelo e tudo faz sentido). Desenvolva uma solução
    # que quebre o "camel casing", usando espaços entre as palavras.
    # Exemplo
    # "camelCasing"  =>  "camel Casing"
    # "mediaFinalAposRecuperacao"  => "media Final Apos Recuperacao"
    # "soma"   =>  "soma"
    # "" =>  ""
    # Argumento: s (string): a palavra original que será processada.
    # Returna: string: um conjunto de palavras, que foram separadas
    # segundo o critério do "camel casing".

    '''for _,letra in enumerate(texto):
        if letra.isupper():
            texto = texto.replace(letra,f' {letra}')
    return texto'''
...
def divisiveis_por_sete_unicos_ordenados(numeros):
    ...
    # Retorna: list: uma lista, de acordo com o critério estabelecido.
    ...
    if len(divisiveis) > 0:
        divisiveis = list(set(divisiveis))
        divisiveis.sort()
    return divisiveis
...
def crescimento_populacional_2(populacao_atual, percentual, aumento, populacao_final):
    # """
    # Determine quantos anos a população da cidade levará para ultrapassar
    #  a população final. Os valores de população atual, percentual de
    # aumento anual, aumento por mudança e da população final são
    # informados como parâmetros da função.

    # Aqui vai um exemplo: suponha que a população é p0 = 1000 no início
    # do ano. A população aumenta regularmente em 2% ao ano e, além disso,
    # 50 novos habitantes por ano vêm morar na cidade.

    # Quantos anos a cidade precisa para ver sua população maior ou
    # igual a p = 1200 habitantes? No final do primeiro ano, haverá:
    # 1000 + 1000 * 0,02 + 50 => 1070 habitantes (o número de habitantes
    # é um inteiro) No final do 2º ano, haverá: 1070 + 1070 * 0,02 + 50 =>
    # 1141 habitantes. No final do 3º ano, haverá: 1141 + 1141 * 0,02 + 50 => 1213
    # Serão necessários 3 anos inteiros.

    # Observação:
    # Não se esqueça de converter o parâmetro de porcentagem em porcentagem no corpo
    # de sua função: se o parâmetro de porcentagem for 2, você deve convertê-lo para
    # 0,02.
    ...
    # Returna: int:
    # a quantidade de anos até se alcançar (ou passar) a
    # população final.

    contador = 0
    while populacao_atual < populacao_final:
        populacao_atual += (populacao_atual*(percentual/100))
        populacao_atual += aumento
        contador += 1
    return contador
...
def fibonacci2(n):
    ...
    # Argumento: n (int): o valor limite do termo da série.
    # Retorna: uma lista de elementos inteiros.
...
def main():
    ...
    test(camel_casing("mediaFinalAposRecuperacao"),
    ...
    test(divisiveis_por_sete_unicos_ordenados(
        [21, 1, 7, 23, 7, 14, 7, 5]), [7, 14, 21])
